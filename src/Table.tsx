    ...
    const filteredData = useMemo(() => {
        return storeData.filter((store) => {
            ...
            if (openFilter === "Open Now") {
                ...
                if (store.name.toLowerCase().includes("danske")) {
                    debugger;
                }
                ...
                if (!(parsedNow >= parsedOpen && parsedNow <= parsedClose)) return false;
            }
            return true;
        });
        ...
        <Card>
            <div className="flex gap-4 mb-4">
                ...
                <Select
                    ...
                    options={[{ value: "Current Day", label: "Current Day" }, ...Object.keys(storeData[0].hours).map(day => ({ value: day, label: day }))]}
                />
                <Select
                    value={openFilter}
                    onChange={setOpenFilter}
                    options={[{ value: "All", label: "All Stores" }, { value: "Open Now", label: "Open Now" }]}
                />
            </div>
